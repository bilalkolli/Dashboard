  ...
  <div class="my-7">
    ...
    <div class="flex gap-10 mt-6 ">
      <template v-for="field in [
        { placeholder: 'California', icon: 'hugeicons:location-01', name: 'Location' },
        { placeholder: 'Industrial Home', icon: 'lucide:home', name: 'Type' }
      ]">
        <div class="relative flex flex-col">
          ...
          <input 
            ...
            :name="field.name" 
            class="pl-12 p-3 rounded-lg border-[1px] border-[#858688] border-opacity-20 bg-light-gray min-w-96 placeholder:text-[#858688]"
          >
        </div>
      </template>

      <Ubutton class="px-9 py-3 text-white" label="Search" :opacity="'bg-opacity-100'" />

      <div class="bg-white flex items-center rounded-lg p-2 px-4">
        <Icon icon="solar:filter-linear" width="1.7em" height="1.7em" color="7C8DA6" />
      </div>
    </div>
  </div>

  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div v-for="(product, index) in products" :key="index" class="rounded-lg p-4 bg-white">
